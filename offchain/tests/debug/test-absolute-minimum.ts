 ...
 * No parameters, no complex types, nothing
 */
...
console.log("═══════════════════════════════════════════════════════");
console.log("🧪 ABSOLUTE MINIMUM VALIDATOR TEST");
console.log("═══════════════════════════════════════════════════════\n");
...
try {
    ...
    const validator = plutusJson.validators.find((v: any) =>
        v.title === "test_absolute_minimum.test_absolute_minimum.spend"
    );
    ...
    const lockTx = await lucid
        ...
        .pay.ToContract(
            address,
            { kind: "inline", value: Data.to(1n) },  // Datum = just 1
    ...
    const spendTx = await lucid
        ...
        .collectFrom([utxo], Data.to(1n))  // Redeemer = just 1
